...
class SeatUserManagementPanel @JvmOverloads constructor(
    context: Context,
    ...
    private fun initView(context: Context) {
        LayoutInflater.from(context).inflate(R.layout.livekit_voiceroom_occupied_seat_manage_view, this, true)
        bindViewId()
    }
    ...
    private fun initUserAvatar() {
        ImageLoader.load(context, imageAvatar, seatInfo.userInfo.avatarURL, R.drawable.livekit_ic_avatar)
    }
    ...
    private fun initDisableAudio() {
        ...
        val isAudienceInMyRoom = TextUtils.equals(liveId, seatInfo.userInfo.liveID)
                && seatInfo.userInfo.role == Role.GENERAL_USER
    ...
    private fun initKickOutSeatView() {
        ...
        val isOwner = TextUtils.equals(liveListStore.liveState.currentLive.value.liveOwner.userID,
            TUIRoomEngine.getSelfInfo().userId)
        val isMyRoomAudience = TextUtils.equals(liveId, seatInfo.userInfo.liveID) && seatInfo.userInfo.role == Role.GENERAL_USER
        layoutKickOutSeat.visibility = if (isMyRoomAudience && isOwner) View.VISIBLE else View.GONE
    ...
    private fun initEndLinkView() {
        ...
        val isAudience = seatInfo.userInfo.role == Role.GENERAL_USER
        layoutEndLink.visibility = if (isSelf && isAudience) View.VISIBLE else View.GONE

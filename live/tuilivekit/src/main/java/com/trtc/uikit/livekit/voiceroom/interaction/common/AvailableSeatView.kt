...
class AvailableSeatView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        inflate(context, R.layout.livekit_voiceroom_available_seat_view, this)
        bindViewId()
    }
    ...
    private fun initLayoutView() {
        layoutRoot.setOnClickListener {
            ...
            if (isOwner) {
                val seatManagementPanel = SeatManagementPanel(context).apply {
                    init(seatInfo)
                }
                PopupDialog(context).apply {
                    setView(seatManagementPanel)
